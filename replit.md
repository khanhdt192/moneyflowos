...
- **Schema:** 3 migrations in `supabase/migrations/`. Tables: `profiles`, `transactions`, `budget_items`, `goals`, `rental_rooms`, `rental_settings`, `rental_billing_cycles`, `rental_room_bills`, `rental_electricity_readings`, `rental_payments`, `invoice_settings`. View: `monthly_summary`. Trigger `handle_new_user` auto-creates a profile row. RLS on every table: `auth.uid() = user_id`.
...
- **Key UI:** AuthCard, MonthSelector, QuickAddDialog, TransactionList, GoalsBoard + GoalCard + WhatIfPanel, RentalBoard (5-tab workspace), ReportsPanel, SettingsPage.
- **Rental module:** Production-grade 5-tab module under `src/components/rental/`. All data persists to Supabase. Tabs: TongQuan (KPI + quick actions), Phong (room table with bill totals), ChotThang (meter readings + draft/confirm billing state machine + payment recording + PDF export), BaoCao (Recharts), CaiDat (cost config, room prices, bank settings, invoice settings — all persisted to DB).
- **Billing state machine:** `draft → confirmed → partial_paid → paid` (also `cancelled`). Global "Chốt hóa đơn" never overwrites confirmed/paid bills. Per-room confirm actions supported. Payments recorded per-payment in `rental_payments`.
- **PDF export:** `src/lib/rental-pdf.ts` uses `jspdf` + `jspdf-autotable`. Generates Vietnamese A4 invoices (single room or all rooms bulk). Includes property info, tenant info, charge breakdown, bank transfer details.
- **Currency:** `formatVND` → "27.000.000 ₫" via `toLocaleString("vi-VN")`. UI is fully Vietnamese.

## One-time setup (user must do)
Run all migrations in order in the Supabase SQL editor (https://supabase.com/dashboard/project/cacvfzxrwwabaqyzelpr/sql/new):
1. `supabase/migrations/0001_init.sql` — core tables, `monthly_summary` view, `handle_new_user` trigger
2. `supabase/migrations/0002_rental_billing.sql` — rental billing tables
3. `supabase/migrations/0003_rental_production.sql` — **REQUIRED for production features**: adds bill status, payments table, invoice settings, bank settings columns. Must be run before using Chốt tháng or PDF export.
All migrations are idempotent (use `IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS`) — safe to re-run.
